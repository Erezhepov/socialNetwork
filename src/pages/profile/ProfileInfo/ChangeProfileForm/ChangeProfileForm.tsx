...
const ChangeProfileForm = ({dispatch, authId, closeModal}: IChangeProfileForm) => {
    ...
    return (
        <form className={s.inputs} onSubmit={handleSubmit(changeProfile)} action="">
            <LabelInputWithErrors label={'Full Name: '} name={'fullName'} type={'text'} register={register} errors={errors} />
            <LabelInputWithErrors label={'About Me: '} name={'aboutMe'} type={'text'} register={register} errors={errors} />
            ...
            <div className={s.input}>
                <label className={s.labelCheckbox}>
                    ...
                    <input onClick={contactHandler} checked={haveContacts} type={'radio'} {...register('haveContact')} />
                </label>
            </div>
            { haveContacts && <>
